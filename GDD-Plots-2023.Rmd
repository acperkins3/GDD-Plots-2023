...
CumulativeRain <- function(data) {
  for (row in 1:nrow(data)) {
    if(row == 1) {
      ...
      data[row, "Cumulative Rainfall (in)"] <- data[(row-1), "Cumulative Rainfall (in)"]
    }
  }
  return(data)
}
...
Merged <- HANExisting %>%
  gather(`S13-S16`, key = "Field", value = "GDDs") %>%
  mutate(Field = case_when(
    Field == "S13-S16" ~ "Hancock (S16-S16)"
  ))

Merged$Field <- factor(Merged$Field, levels = c("Hancock (S16-S16)"))
...
ggplot(Merged, aes(Date, GDDs, group=Field, color=Field)) +
  ...
  annotate("text", x = as.Date("05/10/2023", "%m/%d/%y"), y = 1250, label = "Approximate start of flowering") +
  geom_hline(yintercept = 120, linetype = 'dotted', col = 'blue') +
  annotate("text", x = as.Date("05/10/2023", "%m/%d/%y"), y = 170, label = "Approximate time of emergence") +
...
Merged <- HANExisting %>%
  gather(Hancock, key = "Location", value = "Rainfall")

Merged$Location <- factor(Merged$Location, levels = c("Hancock"))

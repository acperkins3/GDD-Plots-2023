...
NewTemp <- read.csv("ARLWatchdogExisting.csv") %>%
  mutate(Date = as.character(as.Date(Time))) %>%
  separate(Date, c("Year", "Month", "Day")) %>%
  mutate(Date = paste(Month, Day, Year, sep="/")) %>%
  group_by(Date) %>%
  summarize(Min = min(AirTemp),
            Max = max(AirTemp)) %>%
  mutate(JulianDay = yday(as.Date(Date, "%m/%d/%y"))) %>%
  ungroup()

NewRain <- read.csv("ARLWatchdogExisting.csv") %>%
  mutate(Date = as.character(as.Date(Time))) %>%
  separate(Date, c("Year", "Month", "Day")) %>%
  mutate(Date = paste(Month, Day, Year, sep="/")) %>%
  group_by(Date) %>%
  ...
  mutate(JulianDay = yday(as.Date(Date, "%m/%d/%y"))) %>%
  ungroup() %>%
  select(JulianDay, Rain = Rainfall)

NewMerged <- merge(NewTemp, NewRain, by = "JulianDay") %>%
  filter(JulianDay %!in% Existing$JulianDay) %>%
  filter(JulianDay > 130)

New <- merge(Existing, NewMerged, sort=FALSE, all=TRUE) %>%
  mutate(GDDDaily = calc.daily.GDD(max = Max, min = Min)) %>%
  filter(JulianDay >= 130) %>%
...
ggplot(Merged, aes(Date, GDDs, group=Field, color=Field)) +
  ...
  annotate("text", x = as.Date("05/19/2023", "%m/%d/%y"), y = 1250, label = "Approximate start of flowering") +
  geom_hline(yintercept = 120, linetype = 'dotted', col = 'blue') +
  annotate("text", x = as.Date("05/19/2023", "%m/%d/%y"), y = 170, label = "Approximate time of emergence") +
...
ggplot(Merged, aes(Date, GDDs, group=Field, color=Field)) +
  ...
  annotate("text", x = as.Date("05/19/2023", "%m/%d/%y"), y = 2050, label = "~ 35% dry matter silage") +
...
```{r}
WMARSExisting <- read.csv("WMARS_existing.csv") %>%
  select(Date, H703, B501, B601, H702) %>%
  mutate(Date = as.Date(Date, "%m/%d/%y")) %>%
  gather(H703, B501, B601, H702, key = "Field", value = "GDDs") %>%
  mutate(Field = ifelse(Field == "B501", "B501, B502, H101", Field))

WMARSExisting$Field <- factor(WMARSExisting$Field, levels = c("H703","B501, B502, H101", "B601", "H702"))
...
ggplot(WMARSExisting, aes(Date, GDDs, group=Field, color=Field)) +
  ...
  geom_hline(yintercept = 1200, linetype = 'dotted', col = 'blue') +
  annotate("text", x = as.Date("05/29/2023", "%m/%d/%y"), y = 1100, label = "Approximate start of flowering") +
  geom_hline(yintercept = 120, linetype = 'dotted', col = 'blue') +
  ...
  #scale_x_date(limits = c(as.Date("05/1/2023", "%m/%d/%y"), as.Date("05/25/2023", "%m/%d/%y"))) +
  annotate("text", x = as.Date("05/29/2023", "%m/%d/%y"), y = 170, label = "Approximate time of emergence")
...
```{r}
library(lubridate)
moisture <- read.csv("ARLWatchdogExisting.csv") %>%
  mutate(Time = as.POSIXct(Time)) %>%
  mutate(Location = "Arlington (28E)")

ggplot(moisture, aes(Time, SoilMoisture, color = Location)) +
  geom_line() +
  ylab("Soil moisture") +
  xlab("Date") +
  theme_classic() +
...
ggsave("SoilMoisture.png", dpi=200, width = 6, height = 4, units = "in")
```

...
New <- read.csv("WMARS.csv") %>%
  ...
  filter(JulianDay %!in% Existing$JulianDay) %>%
  mutate(Rain = ifelse(is.na(Rain), 0, Rain)) # Sometimes there are missing values
...
Merged <- merge(HANExisting, WMARSExisting, by = "Date", all = TRUE) %>%
  gather(`S13-S16`, M1400, key = "Field", value = "GDDs") %>%
  mutate(Field = case_when(
    Field == "S13-S16" ~ "Hancock (S13-S16)",
    Field == "M1400" ~ "West Madison (M1400)"
  ))

Merged$Field <- factor(Merged$Field, levels = c("Hancock (S13-S16)", "West Madison (M1400)"))
...
ggplot(Merged, aes(Date, GDDs, group=Field, color=Field)) +
  ...
  annotate("text", x = as.Date("05/11/2023", "%m/%d/%y"), y = 1250, label = "Approximate start of flowering") +
  geom_hline(yintercept = 120, linetype = 'dotted', col = 'blue') +
  annotate("text", x = as.Date("05/11/2023", "%m/%d/%y"), y = 170, label = "Approximate time of emergence") +
...
Merged <- WMARSExisting %>%
  gather(M1500, key = "Field", value = "GDDs") %>%
  mutate(Field = case_when(
    Field == "M1500" ~ "West Madison (M1500)"
  ))

Merged$Field <- factor(Merged$Field, levels = c("West Madison (M1500)"))
...
ggplot(Merged, aes(Date, GDDs, group=Field, color=Field)) +
  ...
  annotate("text", x = as.Date("05/11/2023", "%m/%d/%y"), y = 2050, label = "~ 35% dry matter silage") +
...
```{r}
WMARSExisting <- read.csv("WMARS_existing.csv") %>%
  select(Date, H703, B501) %>%
  mutate(Date = as.Date(Date, "%m/%d/%y")) %>%
  gather(H703, B501, key = "Field", value = "GDDs") %>%
  mutate(Field = ifelse(Field == "B501", "B501, B502, H101", Field))

WMARSExisting$Field <- factor(WMARSExisting$Field, levels = c("H703","B501, B502, H101"))
...
ggplot(WMARSExisting, aes(Date, GDDs, group=Field, color=Field)) +
  ...
  geom_hline(yintercept = 1200, linetype = 'dotted', col = 'blue') +
  annotate("text", x = as.Date("05/11/2023", "%m/%d/%y"), y = 1100, label = "Approximate start of flowering") +
  geom_hline(yintercept = 120, linetype = 'dotted', col = 'blue') +
  #geom_hline(yintercept = 2700, linetype = 'dotted', col = 'black') +
  #annotate("text", x = as.Date("05/10/2023", "%m/%d/%y"), y = 2550, label = "~ physiological maturity") +
  #scale_x_date(limits = c(as.Date("05/1/2023", "%m/%d/%y"), as.Date("05/25/2023", "%m/%d/%y"))) +
  annotate("text", x = as.Date("05/11/2023", "%m/%d/%y"), y = 170, label = "Approximate time of emergence")
...
Merged <- merge(HANExisting, WMARSExisting, by = "Date", all = TRUE) %>%
  gather(Hancock, `West Madison`, key = "Location", value = "Rainfall")

Merged$Location <- factor(Merged$Location, levels = c("Hancock", "West Madison"))
